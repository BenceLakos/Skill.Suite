...
readonly SESSION=Fibonacci
TAG=${TAG:-1}
BASE_IMAGE=${BASE_IMAGE:-ghcr.io/bencelakos/skill-suite-judge-base:9.0}
REGISTRY=${REGISTRY:-}
...
# Pinned: verification/expected/*.json and the documented judge-run invocations name this
# image, so it must not follow a project rename.
IMAGE_NAME=${IMAGE_NAME:-fibonacci-judge}
BLACKBOX_IMAGE_NAME=${BLACKBOX_IMAGE_NAME:-fibonacci-blackbox-judge}
