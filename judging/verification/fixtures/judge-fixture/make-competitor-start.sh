...
KIND=${1:-whitebox}
case "${KIND}" in
    whitebox) SWAPPED="JudgeFixture.Services" ;;
    blackbox) SWAPPED="JudgeFixture.UnitTests" ;;
...
if dotnet build "${WORK}/JudgeFixture.sln" -c Release --nologo >"${WORK}/build.log" 2>&1; then
